...
export default class Grid {
  ...
  public render() {
    ...
    const startMainX =
      Math.ceil(this.settings.viewbox.x.min / (resX * 10)) * 10 -
      Math.floor(this.settings.viewbox.x.min / resX);
    const startMainY =
      Math.ceil(this.settings.viewbox.y.min / (resY * 10)) * 10 -
      Math.floor(this.settings.viewbox.y.min / resY);
    ...
    for (let i = 0; i * 10 + startMainX < this.gridNumX; i++) {
      ...
      const numberY =
        ...
        this.svg.clientHeight * (-this.settings.viewbox.y.min / height);
    ...
    for (let i = 0; i * 10 + startMainY < this.gridNumY; i++) {
      ...
      const numberX =
        this.svg.clientWidth * (-this.settings.viewbox.x.min / width);
