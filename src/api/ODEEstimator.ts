...
export default class ODEEstimator {
  ...
  public stepFrom(
    ...
    if (normalize) {
      const mag =
        Math.sqrt(estimator.stepX ** 2 + estimator.stepY ** 2) / speed;
      estimator.stepX /= mag;
      estimator.stepY /= mag;
    }
  ...
  private stepFromRK2(estimator: ODEEstimatorObject, speed: number) {
    ...
    const x_k2 = this.dxFunction(
      x + 0.5 * speed * x_k1,
      y + 0.5 * speed * y_k1
    );
    const y_k2 = this.dyFunction(
      x + 0.5 * speed * x_k1,
      y + 0.5 * speed * y_k1
    );
  ...
  private stepFromRK4(estimator: ODEEstimatorObject, speed: number) {
    ...
    const x_k3 = this.dxFunction(
      x + 0.5 * speed * x_k2,
      y + 0.5 * speed * y_k2
    );
    const y_k3 = this.dyFunction(
      x + 0.5 * speed * x_k2,
      y + 0.5 * speed * y_k2
    );
  ...
  private loadPolar() {
    // @ts-ignore
    const dr = MastonToJSFunction(
      mathlive.latexToAST(store.state.settings.drString),
      ["r", "t"]
    );
    // @ts-ignore
    const dt = MastonToJSFunction(
      mathlive.latexToAST(store.state.settings.dtString),
      ["r", "t"]
    );
  ...
  private loadCartesian() {
    try {
      // @ts-ignore
      const dx = MastonToJSFunction(
        mathlive.latexToAST(store.state.settings.dxString)
      );
      // @ts-ignore
      const dy = MastonToJSFunction(
        mathlive.latexToAST(store.state.settings.dyString)
      );
      // Test validity
      dx(0, 0);
      dy(0, 0);
