...
export default class CachedFunction {
    ...
    public getdxdy(x: number, y: number): [number, number, boolean] {
        ...
        const index = Math.floor(this.width * normX) + this.width * Math.floor(this.height * normY);
        return [
            this.cache[index * 4],
    ...
    public getImage() {
        return Uint8Array.from(this.cache, (value, i) =>  {
            const id = i % 4;
            if (id === 0) {
                return  Math.floor((value - this.dxMin) / (this.dxMax - this.dxMin) * 255);
            } else if (id === 1) {
                return  Math.floor((value - this.dyMin) / (this.dyMax - this.dyMin) * 255);
            } else if (id === 2) {
                return 0;
            } else {
                return value * 255;
