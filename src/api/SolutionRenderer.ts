...
export default class SolutionRenderer {
  ...
  constructor(
    ...
    canvas.addEventListener("click", (e: MouseEvent) => {
      ...
      const x =
        ...
        (e.offsetX / width) *
          (this.settings.viewbox.x.max - this.settings.viewbox.x.min);
      const y =
        this.settings.viewbox.y.max -
        (e.offsetY / height) *
          (this.settings.viewbox.y.max - this.settings.viewbox.y.min);
  ...
  setWidth() {
    this.svg.style.setProperty(
      "--solution-width",
      this.settings.solutionWidth.toString()
    );
  }
  ...
  addSolution(x: number, y: number) {
    ...
    for (let i = 0; i < length; i++) {
      for (let j = 0; j <= this.subSteps; j++) {
        this.ODEEstimator.stepFrom(
          estimation,
          stepSize / (this.subSteps + 1),
          false,
          this.method
        );
  ...
  renderSolution(solution: Solution) {
    ...
    const pathForward = solution.dForwards
      .map(d => "L " + this.formatCoordinates(d))
      .join(" ");
    const pathBackward = solution.dBackwards
      .map(d => "L " + this.formatCoordinates(d))
      .join(" ");
  ...
  private formatCoordinates([x, y]: [number, number]) {
    const xPer =
      (x - this.settings.viewbox.x.min) /
      (this.settings.viewbox.x.max - this.settings.viewbox.x.min);
    const yPer =
      (y - this.settings.viewbox.y.min) /
      (this.settings.viewbox.y.max - this.settings.viewbox.y.min);
